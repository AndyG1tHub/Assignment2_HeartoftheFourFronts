...
- **EnemyAI.moveOrAttackBase**: 每次敌人更新时都重新检查目标是否还存在、是否还能攻击；未进入攻击范围时立即调用 `PathFinder.findPath` 重新计算到基地的最短路径。
- **PathFinder.findPath**: 使用 BFS 四方向搜索，返回从当前格到目标格的最短路径列表。
- **PathFinder.findPathIgnoringBuildings**: 当所有可走路径被建筑堵住时，临时忽略建筑但仍避开障碍物，找出通向基地的理论最短路径。
- **PathFinder.getNeighbors**: 每次只扩展上下左右四个邻居，并按到目标的直线距离从近到远排序，禁止敌人跨越格子角落。
- **Enemy.followPath**: 不缓存完整旧路径，只在当前相邻格移动完成前保留下一格目标；敌人平滑移动到相邻格中心，路径仍然只允许上下左右四方向。
...
敌人记录 `facingUp`、`facingDown`、`facingLeft`、`facingRight` 四个朝向变量，每次朝相邻格移动时更新本次动作方向。攻击范围使用八方向格子距离判定，但移动寻路仍只走上下左右。
